      ...
      >
        <Link to={`/product/${product.id}`} className="flex gap-4 p-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <h3 className="font-semibold text-gray-900 mt-0.5 truncate">{product.name}</h3>
            ...
            <div className="flex items-center justify-between mt-3">
              ...
              <div className="flex gap-2">
                <button onClick={handleWishlist} className={`p-2 rounded-lg border ${isWishlisted ? 'border-pink-200 bg-pink-50 text-pink-500' : 'border-gray-200 hover:border-gray-300'}`}>
                  <Heart className={`w-4 h-4 ${isWishlisted ? 'fill-pink-500' : ''}`} />
                </button>
                <button onClick={handleAddToCart} className="flex items-center gap-2 px-3 py-2 bg-violet-600 text-white rounded-lg text-sm font-medium hover:bg-violet-700 transition-colors">
          ...
          <div className="absolute bottom-3 left-0 right-0 flex justify-center gap-2 opacity-0 group-hover:opacity-100 transition-all translate-y-2 group-hover:translate-y-0">
            <button
              ...
              className={`w-9 h-9 rounded-full shadow-lg flex items-center justify-center ${
                isWishlisted ? 'bg-pink-500 text-white' : 'bg-white text-gray-700'
