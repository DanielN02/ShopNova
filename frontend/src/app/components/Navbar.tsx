...
import {
  ShoppingCart, Search, Heart, Bell, User, Menu, X,
  LogOut, Settings, Package, LayoutDashboard, ChevronDown
} from 'lucide-react';
...
export function Navbar() {
  ...
  const { isAuthenticated, currentUser, logout, cartItems, notifications, markAllRead, setSearchQuery, searchQuery } = useStore();
  ...
  const navigateToCatalog = useCallback((query: string) => {
    if (query.trim()) {
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              ...
              onClick={() => navigate(isAuthenticated ? '/dashboard/wishlist' : '/login')}
            >
                    ...
                    >
                      ...
                      <div className="max-h-72 overflow-y-auto">
                        {notifications.length === 0 ? (
                          ...
                          notifications.map(n => (
                            <div key={n.id} className={`p-4 border-b border-gray-50 last:border-0 ${!n.read ? 'bg-violet-50' : ''}`}>
                              <div className="flex gap-2 items-start">
                                <div className={`w-2 h-2 rounded-full mt-1.5 shrink-0 ${!n.read ? 'bg-violet-500' : 'bg-gray-300'}`} />
                                <div>
                    ...
                    >
                      <div className="p-4 border-b border-gray-100">
                        ...
                        <span className={`mt-1 inline-block text-xs px-2 py-0.5 rounded-full font-medium ${
                          currentUser?.role === 'admin' ? 'bg-orange-100 text-orange-700' : 'bg-violet-100 text-violet-700'
                        }`}>
                          {currentUser?.role}
                        </span>
                      </div>
        ...
        <AnimatePresence>
          {menuOpen && (
            ...
            >
              ...
              {!isAuthenticated && (
                <>
                  <Link to="/login" className="flex items-center px-3 py-2.5 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50" onClick={() => setMenuOpen(false)}>
                    <User className="w-4 h-4 mr-2" /> Sign In
                  </Link>
                  <Link to="/register" className="flex items-center px-3 py-2.5 rounded-lg text-sm font-medium text-white bg-violet-600" onClick={() => setMenuOpen(false)}>
