...
const STATUS_CONFIG: Record<OrderStatus, { icon: React.ElementType; color: string; bg: string; label: string }> = {
  pending: { icon: Clock, color: 'text-purple-600', bg: 'bg-purple-100', label: 'Pending' },
        ...
        <div className="flex gap-6">
          ...
          <aside className="hidden md:flex flex-col w-60 shrink-0 gap-2">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-5 text-center mb-2">
              ...
              />
              <p className="font-bold text-gray-900 mt-3">{currentUser?.name}</p>
              ...
              >
                ...
                {item.badge !== undefined && item.badge > 0 && (
                  <span className={`ml-auto text-xs font-bold px-2 py-0.5 rounded-full ${
                    activeView === item.key ? 'bg-white/20 text-white' : 'bg-violet-100 text-violet-700'
                  }`}>
                    {item.badge}
                  </span>
                )}
              ...
              {SIDEBAR_ITEMS.map(item => (
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium whitespace-nowrap transition-colors ${
                    activeView === item.key ? 'bg-violet-600 text-white' : 'bg-white text-gray-600 border border-gray-200'
                  }`}
                >
                  <item.icon className="w-4 h-4" />
                  {item.label}
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                ...
                <div className="bg-white rounded-2xl border border-gray-100 p-5">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button onClick={() => setActiveView('orders')} className="text-xs text-violet-600 flex items-center gap-1">
                  ...
                  <div className="bg-white rounded-2xl border border-gray-100 p-5">
                    ...
                    <div className="space-y-3">
                      {notifications.map(n => (
                        ...
                        >
                          <div className={`w-2 h-2 rounded-full mt-2 shrink-0 ${!n.read ? 'bg-violet-500' : 'bg-gray-300'}`} />
                          <div>
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
                ...
                {ordersLoading ? (
                  ...
                  <div className="text-center py-16 bg-white rounded-2xl border border-gray-100">
                    ...
                    <Link to="/catalog" className="mt-4 inline-flex items-center gap-2 px-6 py-2.5 bg-violet-600 text-white rounded-full text-sm font-medium">
                      Start Shopping
                    </Link>
                  </div>
                ) : (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {wishlistProducts.map(product => (
                      <div key={product.id} className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
                        ...
                        <div className="p-4">
                          <p className="text-xs text-violet-600 font-medium">{product.category}</p>
                          <h3 className="font-semibold text-gray-900 mt-0.5">{product.name}</h3>
                          <p className="font-bold text-gray-900 mt-2">${product.price.toFixed(2)}</p>
                          <Link to={`/product/${product.id}`} className="mt-3 block text-center py-2 bg-violet-600 text-white rounded-xl text-sm font-medium hover:bg-violet-700 transition-colors">
                            View Product
                          </Link>
                        </div>
                      </div>
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
                ...
                <div className="bg-white rounded-2xl border border-gray-100 p-6">
                  <h2 className="font-bold text-gray-900 mb-5 flex items-center gap-2">
                    <User className="w-4 h-4 text-violet-600" /> Profile Information
                  </h2>
                  <div className="flex items-center gap-4 mb-6">
                    <img src={currentUser?.avatar} alt="" className="w-20 h-20 rounded-full object-cover border-4 border-violet-100" />
                ...
                <div className="bg-white rounded-2xl border border-gray-100 p-6">
                  ...
                  <div className="space-y-3">
                    {[
                      ...
                      { label: 'Promotions & deals', desc: 'Receive exclusive offers and sale alerts' },
                      { label: 'Newsletter', desc: 'Weekly curated product picks' },
                    ].map(pref => (
                      <label key={pref.label} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl cursor-pointer">
                        <div>
