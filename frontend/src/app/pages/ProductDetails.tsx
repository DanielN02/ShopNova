...
import {
  Star, Heart, ShoppingCart, Truck, ShieldCheck, RefreshCw,
  ChevronRight, Plus, Minus, Share2, Tag
} from 'lucide-react';
  ...
  if (!product) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4">
        ...
        <button onClick={() => navigate('/catalog')} className="px-6 py-3 bg-violet-600 text-white rounded-full">
          Back to Catalog
        </button>
      </div>
    );
      ...
      <div className="bg-white border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <div className="flex items-center gap-2 text-sm text-gray-500">
            ...
            <ChevronRight className="w-4 h-4" />
            <Link to={`/catalog?category=${product.category}`} className="hover:text-violet-600 transition-colors">{product.category}</Link>
            <ChevronRight className="w-4 h-4" />
            ...
            <div className="relative bg-gray-50 aspect-square lg:aspect-auto lg:min-h-[480px] overflow-hidden">
              ...
              <button
                ...
                className={`absolute top-6 right-6 w-11 h-11 rounded-full flex items-center justify-center shadow-lg transition-all ${
                  isWishlisted ? 'bg-pink-500 text-white' : 'bg-white text-gray-500 hover:bg-pink-50 hover:text-pink-500'
                }`}
              >
            ...
            <div className="p-8 lg:p-10 flex flex-col">
              ...
              <div className="flex items-center gap-2 flex-wrap mt-4">
                ...
                {product.tags.map(tag => (
                  <Link key={tag} to={`/catalog?tag=${tag}`} className="px-2.5 py-1 bg-gray-100 text-gray-600 rounded-lg text-xs hover:bg-violet-100 hover:text-violet-700 transition-colors">
                    {tag}
                  </Link>
                ))}
              </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-8 p-6 bg-gray-50 rounded-2xl mb-6">
                  <div className="text-center">
                    <p className="text-6xl font-black text-gray-900">{product.rating}</p>
